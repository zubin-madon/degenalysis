...
function Login() {
  const moralisCtx = useCustomMoralisContext();
    ...
    const login = async () => {
      ...
      if (!moralisCtx.isAuthenticated) {
        ...
        await moralisCtx.authenticate({signingMessage: "Sign in to Degenalysis! This does not involve any transaction cost. This DOES NOT allow Degenalysis to access your funds."})
          .then(function (user) {
  ...
  return (
    <Fragment>
      <button onClick={!moralisCtx.isAuthenticated?() => login():()=>logOut()}
        type="button"
        className="mt-1 text-gray-900 bg-white hover:bg-purple-400 border-4 border-purple-600 focus:ring-4 focus:outline-none focus:ring-purple-400 font-medium rounded-lg text-sm px-5 py-2 text-center inline-flex items-center mr-2 mb-2"
      >
        <svg
        ...
        {!moralisCtx.isAuthenticated?"Connect":"LogOut "+moralisCtx.user.get("ethAddress").slice(0,6)}
      </button>
      <div>
